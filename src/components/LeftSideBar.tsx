        ...
        <div className='h-screen sticky top-0 flex flex-col justify-between pt-2 pb-8'>
            ...
            <div className=' flex flex-col gap-4 text-lg items-center xxl:items-start'>
                <Link href='/' className='p-2 rounded-full hover:bg-[#181818] '>
                    <OptimizerImage src='/icons/logo.svg' alt='logo' height={24} width={24} />
                </Link>
                {/* Menu list */}
                <div className='flex flex-col gap-2'>
                    {menuList.map((item) => (
                       <Link href={item.link} className='p-2 rounded-full hover:bg-[#181818] flex items-center gap-4' key={item.id}>
                           <OptimizerImage src={`icons/${item.icon}`} alt={`${item.name}`} width={24} height={24} />
                ...
                <Link href='/' className='bg-white text-black rounded-full w-12 h-12 flex items-center justify-center xxl:hidden'>
                    <OptimizerImage src='icons/post.svg' alt='new post icon' width={24} height={24} />
                </Link>
            ...
            <div className='flex items-center justify-between'>
                <div className='flex items-center gap-2 '>
                    <div className='w-10 h-10 relative rounded-full overflow-hidden'>
                        <OptimizerImage src='general/avatar.jpg' alt='oleksandr zhynkin' width={100} height={100} transformation={true} />
                    </div>
